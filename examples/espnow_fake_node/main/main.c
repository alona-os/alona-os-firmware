/*
 * Bench-only fake ESP-NOW node: fixed channel, no Wi-Fi association.
 * See docs/gateway-setup.md.
 */
...
static const char *TAG = "alona_fake";
...
static esp_err_t wifi_init_channel_only(void) {
    ...
    if (esp_wifi_get_mac(WIFI_IF_STA, mac) == ESP_OK) {
        ESP_LOGI(TAG, "fake node sta mac %02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1], mac[2],
                 mac[3], mac[4], mac[5]);
    }
...
static void send_loop_task(void *pv) {
    ...
    for (;;) {
        int n = snprintf(payload, sizeof(payload),
                         "{\"version\":1,\"device_id\":\"%s\",\"readings\":{\"temperature_c\":%.2f,"
                         "\"relative_humidity_pct\":54.30}}",
                         ALONA_DEFAULT_DEVICE_ID, (double)temp_c);
        if (n <= 0 || (size_t)n >= sizeof(payload)) {
            ESP_LOGE(TAG, "payload snprintf failed");
            vTaskDelay(pdMS_TO_TICKS(ALONA_ESPNOW_SEND_INTERVAL_MS));
            continue;
        }

        ESP_LOGI(TAG, "sending esp-now json len=%d", n);
        ESP_LOGI(TAG, "payload: %s", payload);

        esp_err_t err = esp_now_send(k_gateway_mac, (const uint8_t *)payload, (size_t)n);
        if (err != ESP_OK) {
...
void app_main(void) {
    ESP_LOGI(TAG, "boot fake esp-now node (channel-only, no Wi-Fi join)");
    ...
    xTaskCreate(send_loop_task, "alona_fake_send", 4096, NULL, 5, NULL);
}
